(function ($) {
  ...
  LYDIAProcess.prototype.sendRequest = function () {
    if (this.isRunning == false) {
      this.isRunning = true;
      $.post(this.baseUrl + "api/request/do.json", this.data, function (data) {
        if (data.error == 0) {
  ...
  $.fn.payWithLYDIA = function (data) {
    ...
    for (var i = 0; i < lydiaProcess.configKey.length; i++) {
      if (
        lydiaProcess.configToSkip.indexOf(lydiaProcess.configKey[i]) < 0 &&
        data[lydiaProcess.configKey[i]] != undefined
      ) {
    ...
    if (data.render) {
      ...
      $(this).html(
        '<a href="#" onclick="return false;"><img class="lydia_payment_button" src="' +
          lydiaProcess.baseUrl +
          'assets/img/paymentbutton.png" height="40" /></a>'
      );
    }
