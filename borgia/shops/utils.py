...
DEFAULT_PERMISSIONS_ASSOCIATES = [
    "add_user",
    "view_user",
    "view_shop",
    "add_product",
    ...
    for shop in shop_managed:
        shop_tree.append(
            simple_lateral_link(
                ...
                "briefcase",
                "lm_workboard",
                reverse("url_shop_workboard", kwargs={"shop_pk": shop.pk}),
            )
        )
    return shop_tree
...
def shops_lateral_menu(nav_tree, user, shop):
    ...
    if user.has_perm("shops.view_shop"):
        nav_tree.append(
            simple_lateral_link(
                ...
                "user",
                "lm_shop_checkup",
                reverse("url_shop_checkup", kwargs={"shop_pk": shop.pk}),
            )
        )
    ...
    if user.has_perm("finances.view_sale"):
        nav_tree.append(
            simple_lateral_link(
                ...
                "shopping-cart",
                "lm_sale_list",
                reverse("url_sale_list", kwargs={"shop_pk": shop.pk}),
            )
        )
    ...
    if user.has_perm("shops.view_product"):
        nav_tree.append(
            simple_lateral_link(
                ...
                url=reverse("url_product_list", kwargs={"shop_pk": shop.pk}),
            )
        )
    ...
    if user.has_perm("stocks.view_stockentry"):
        nav_tree.append(
            simple_lateral_link(
                ...
                "list",
                "lm_stockentry_list",
                reverse("url_stockentry_list", kwargs={"shop_pk": shop.pk}),
            )
        )

    # TODO create another perm
    if user.has_perm("stocks.view_stockentry"):
        nav_tree.append(
            simple_lateral_link(
                "Entrées factures",
                "file",
                "lm_billsentry_list",
                reverse("url_billsentry_list", kwargs={"shop_pk": shop.pk}),
            )
        )
    ...
    if user.has_perm("stocks.view_inventory"):
        nav_tree.append(
            simple_lateral_link(
                ...
                "list",
                "lm_inventory_list",
                reverse("url_inventory_list", kwargs={"shop_pk": shop.pk}),
            )
        )
    ...
    subs = []
    groups = [
        Group.objects.get(name="chiefs-" + shop.name),
        ...
        if user.has_perm(get_permission_name_group_managing(group)):
            subs.append(
                simple_lateral_link(
                    ...
                    reverse("url_group_update", kwargs={"group_pk": group.pk}),
                )
            )
    if len(subs) > 1:
        nav_tree.append(
