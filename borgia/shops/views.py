...
from .serializers import (
    ...
    DeleteShopSerializer,
    OperatorSaleModuleSerializer,
...
class ShopCreate(LoginRequiredMixin, PermissionRequiredMixin, BorgiaFormView):
    ...
    def form_valid(self, form):
        ...
        shop = Shop.objects.create(
            ...
            correcting_factor_activated=form.cleaned_data[
...
class ShopUpdate(ShopMixin, BorgiaFormView):
    ...
    def form_valid(self, form):
        ...
        self.shop.correcting_factor_activated = form.cleaned_data[
            "correcting_factor_activated"
        ]
        self.shop.save()
        return super().form_valid(form)
...
def create_shop_api_function(
    shop_name,
    shop_description,
    shop_image,
    correcting_factor_activated=True,
    shop_color="#F4FA58",
):
    if (
        shop_name
        and shop_description
        and shop_color
        and shop_image
        and (correcting_factor_activated != None)
    ):
        Shop.objects.create(
            name=shop_name,
...
def create_product_api_function(
    product_name,
    price_is_manual,
    manual_price,
    shop_id,
    ...
    if (
        product_name
        and (price_is_manual != None)
        and shop_id
        and (is_active != None)
        and correcting_factor
        and product_image
        and manual_price
    ):
...
class CreateProductView(views.APIView):
    ...
    def post(self, request):
        ...
        create_product_api_function(
            productMap["product_name"],
            productMap["price_is_manual"],
            productMap["manual_price"],
            productMap["shop_id"],
            productMap["is_active"],
            product_unit,
            productMap["correcting_factor"],
...
def update_product_api_function(
    product_id,
    product_name=None,
