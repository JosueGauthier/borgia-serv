...
events_patterns = [
    path('events/', include([
        ...
        path('<int:pk>/', include([
            ...
            path('self_registration/', EventSelfRegistration.as_view(),
                 name='url_event_self_registration'),
            path('users/', EventManageUsers.as_view(),
                 name='url_event_manage_users'),
            path('users/<user_pk>/remove/', EventRemoveUser.as_view(),
                 name='url_event_remove_user'),
            path('users/<int:user_pk>/change_weight',
                 EventChangeWeight.as_view(), name='url_event_change_weight'),
            path('xlsx/download/', EventDownloadXlsx.as_view(),
                 name='url_event_download_xlsx'),
            path('xlsx/upload/', EventUploadXlsx.as_view(),
                 name='url_event_upload_xlsx')
        ]))
    ]))
]
