...
class HistorySaleUserSerializer(serializers.BaseSerializer):
    def to_representation(self, instance):
        ...
        startDay = (
            Sale.objects.filter(sender=instance.id)
            .values("datetime")
            .first()["datetime"]
        )
        lastDay = (
...
class SaleStatSerializer(serializers.ModelSerializer):
    ...
    def get_user_sales(self, obj):
        totalsale = Sale.objects.filter(sender__username="73An220").aggregate(
            Sum("module_id")
        )
        return totalsale
        ...
        return {
            ...
            "montant_achats": float(
                SaleProduct.objects.filter(
                    sale__sender__username=instance.username
                ).aggregate(Sum("price"))["price__sum"]
                or 0
            ),
            "qte_achats": SaleProduct.objects.filter(
                sale__sender__username=instance.username
            ).count(),
            "nb_ref_achetee": SaleProduct.objects.filter(
                sale__sender__username=instance.username
            )
            .values("product")
            .distinct()
            ...
            "montant_magasins": [
                {
                    ...
                    "shop_image": shop_data.get("image"),
                    "qte_user_achats": SaleProduct.objects.filter(
                        sale__sender__username=instance.username, sale__shop=i
                    ).count(),
                    "montant_achats": float(
                        SaleProduct.objects.filter(
                            sale__sender__username=instance.username, sale__shop=i
                ...
                if (
                    shop_data := Shop.objects.filter(id=i)
                    .values("name", "image")
                    .first()
                )
                is not None
...
class UserRankByShopSerializer(serializers.BaseSerializer):
    def to_representation(self, instance):
        return {
            ...
            "montant_achats_par_shop": float(
                SaleProduct.objects.filter(
                    sale__sender__username=instance.username,
                    sale__shop=self.context.get("shop_id"),
                ).aggregate(Sum("price"))["price__sum"]
                or 0
            ),
        }
...
class UserRankByProductSerializer(serializers.BaseSerializer):
    def to_representation(self, instance):
        return {
            ...
            "campus": instance.campus,
            "promotion": instance.year,
            "montant_achats_par_produit": float(
                SaleProduct.objects.filter(
                    sale__sender__username=instance.username,
...
def ProductToptenUserView(id):
    ...
    sortedList = sorted(
        serializer.data, key=itemgetter("montant_achats_par_produit"), reverse=True
    )[:10]
    return sortedList
